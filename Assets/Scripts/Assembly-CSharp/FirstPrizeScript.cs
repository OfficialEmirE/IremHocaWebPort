...
public class FirstPrizeScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			if (!windowScript.broken && Vector3.Distance(base.transform.position, windowScript.transform.position) < 5f && prevSpeed - agent.velocity.magnitude > 15f)
			{
				GameControllerScript component = GameObject.FindWithTag("GameController").GetComponent<GameControllerScript>();
				if (component.baldiScrpt.isActiveAndEnabled & !windowScript.broken)
				{
					component.baldiScrpt.Hear(windowScript.transform.position, 7f);
				}
