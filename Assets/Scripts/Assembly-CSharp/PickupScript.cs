...
public class PickupScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (Input.GetMouseButtonDown(0) && Time.timeScale != 0f && Physics.Raycast(Camera.main.ScreenPointToRay(new Vector3(Screen.width / 2, Screen.height / 2, 0f)), out var hitInfo))
		{
			...
			else if ((hitInfo.transform.name == "Pickup_Whistle") & (Vector3.Distance(player.position, base.transform.position) < 10f))
			{
				hitInfo.transform.gameObject.SetActive(value: false);
				gc.CollectItem(11);
			}
			else if ((hitInfo.transform.name == "Pickup_ChalkEraser") & (Vector3.Distance(player.position, base.transform.position) < 10f))
			{
				hitInfo.transform.gameObject.SetActive(value: false);
				gc.CollectItem(12);
			}
