...
public class MathGameScript : MonoBehaviour
{
	...
	private string[] hintText = new string[3] { "HER YANLIŞ CEVABINDA DAHADA SİNİRLİ HALE GELİCEK!", "HER AÇTIĞIN KAPIYI DUYUYOR!", "SENİ YAKALACAK!" };

	private string[] endlessHintText = new string[3] { "Fena Değil...", "biraz daha fazla soruları cevapla.", "zekanın kelini öpeyim." };
	...
	private void NewProblem()
	{
		...
		if (problem <= 3)
		{
			...
			if (((gc.mode == "story") & (problem <= 2 || gc.notebooks <= 1)) || ((gc.mode == "endless") & (problem <= 2 || gc.notebooks != 2)))
			{
				...
				if (sign == 0)
				{
					...
					questionText.text = "MAT PROBLEMI" + problem + ": \n \n" + num1 + "+" + this.num2 + "=";
					QueueAudio(bal_plus);
				}
				else if (sign == 1)
				{
			...
			else
			{
				...
				if (sign == 0)
				{
					questionText2.text = "MAT PROBLEMI" + problem + ": \n" + num1 + "+(" + this.num2 + "X" + num3 + "=";
				}
				else if (sign == 1)
				{
					questionText2.text = "MAT PROBLEMI" + problem + ": \n (" + num1 + "/" + this.num2 + ")+" + num3 + "=";
				...
				if (sign == 0)
				{
					questionText3.text = "MAT PROBLEMI" + problem + ": \n" + num1 + "+(" + this.num2 + "X" + num3 + "=";
				}
				else if (sign == 1)
				{
					questionText3.text = "MAT PROBLEMI" + problem + ": \n (" + num1 + "/" + this.num2 + ")+" + num3 + "=";
		...
		if (!gc.spoopMode)
		{
			questionText.text = "Vay Canına Helal!";
		}
	...
	{
		if (playerAnswer.text == "187")
		{
			StartCoroutine(CheatText("HERŞEY BURDA BAŞLADI (şüpheli)"));
			Thread.Sleep(1);
            SceneManager.LoadSceneAsync("TestRoom");
		}
		else if (playerAnswer.text == "2071")
		{
			gc.Fliparoo();
        }
        else if (playerAnswer.text == "67")
        {
            StartCoroutine(CheatText("allah rızası için psikoloğa görüş"));
            Thread.Sleep(1);
			Application.Quit();
        }
        if (problem > 3)
